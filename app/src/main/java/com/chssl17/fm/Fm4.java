...
public class Fm4 extends BaseFm {
    ...
    private com.chssl17.view.TagGroup m_tag1;

    @ViewInject(R.id.tag2)
    private com.chssl17.view.TagGroup m_tag2;

    @ViewInject(R.id.tag3)
    private com.chssl17.view.TagGroup m_tag3;
    ...
    private void fmOnClick(View v) {
        ...
        for (int i = 0;i < mTvLists.size();i++)
        {
            if (v.getId() == mTvLists.get(i).getId())
            {
                ...
                mTvLists.get(i).setTextColor(getResources().getColor(R.color.white));
            }
        }
        getTags(num);
    }
